...
Everything the [README](../README.md) leaves out: the full command list, how output
behaves at a terminal, each feature in detail, and the parts of authentication that only
matter when something goes wrong. `mcpdial --help` covers the same command surface more
briefly, and [AGENTS.md](AGENTS.md) is the version written for programs.
...
mcpdial completions SHELL        a completion script; see Shell completions above
```
...
colour off. A snapshot test holds `Plain` to its word; see [development.md](development.md).
...
machine: [`client-metadata.json`](client-metadata.json), published by
...
## Tracing what went over the wire
...
The files mcpdial keeps, and where, are listed in the
[README](../README.md#where-things-live). The `run` directory among them is created
mode 0700, so only its owner can reach a running server through it.
...
|---|---|
| `MCPDIAL_HOME` | The config directory, instead of the default |
